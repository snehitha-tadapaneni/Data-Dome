...
library(caret)        # Data splitting, pre-processing, and evaluation
library(ggplot2)      # Data visualization
library(pROC)         # For ROC and AUC
library(e1071)        # For tuning the KNN hyperparameters

# Load the dataset (assume 'data_updated' is available)
# Clean and prepare the data
data_updated <- data_updated %>%
  mutate_if(is.character, as.factor) %>%  # Convert character columns to factors
  mutate_if(is.factor, as.numeric) %>%    # Convert factor columns to numeric for PCA
  drop_na()  # Drop rows with NA values

# Select numeric variables for PCA
pca_data <- data_updated[, sapply(data_updated, is.numeric)]
...
# Scree Plot
plot(pca_result, type = "l", main = "Scree Plot", col = "blue")

# Extract cumulative variance and choose optimal number of PCs (90% variance)
explained_variance <- cumsum(pca_result$sdev^2 / sum(pca_result$sdev^2))
optimal_pcs <- min(which(explained_variance >= 0.9))  # Choose number of PCs to explain 90% of variance
print(paste("Optimal number of principal components (90% variance):", optimal_pcs))

# PCA-transformed data
pca_scores <- as.data.frame(pca_result$x[, 1:optimal_pcs])  # Use the top 'optimal_pcs' principal components
pca_scores$Installs_Category <- data_updated$Installs_Category

# Split PCA-transformed data into train/test sets (75% train, 25% test)
set.seed(42)
trainIndex <- createDataPartition(pca_scores$Installs_Category, p = 0.75, list = FALSE)
X_train <- pca_scores[trainIndex, 1:optimal_pcs]
X_test <- pca_scores[-trainIndex, 1:optimal_pcs]
y_train <- pca_scores$Installs_Category[trainIndex]
y_test <- pca_scores$Installs_Category[-trainIndex]
...
knn_tuned <- train(
  ...
  tuneGrid = data.frame(k = seq(1, 20, by = 1))  # Tune for k = 1 to 20
)
...
confusion_matrix <- confusionMatrix(as.factor(knn_model), as.factor(y_test))
print(confusion_matrix)

# Plot Accuracy vs k
accuracy_results <- data.frame(k = knn_tuned$results$k, Accuracy = knn_tuned$results$Accuracy)
...
predicted <- as.factor(knn_model)
actual <- as.factor(y_test)

precision <- posPredValue(predicted, actual, positive = "High")
recall <- sensitivity(predicted, actual, positive = "High")
