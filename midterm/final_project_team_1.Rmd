...
# Calculate accuracy
train_accuracy <- sum(train_predictions == train_data$Installs_Category) / nrow(train_data)
test_accuracy <- sum(test_predictions == test_data$Installs_Category) / nrow(test_data)
...
High Dimensionality: With 41 variables, random forest handles many features better and can identify the most important ones.
Feature Importance: Random forest provides a ranking of feature importance, helping us understand which variables influence the Installs_Category.
...
Feature Selection: Using the top 5 categories as predictors, which are highly correlated with app performance metrics such as installs, ratings, and reviews.
Model Objective: Accurately predict the number of installs by capturing the complex and nonlinear relationships between features using a Random Forest model.
Evaluation Metrics: Assess the model's performance using metrics such as Mean Squared Error (MSE), R-squared, and visualization of feature importance to ensure the model’s predictions are interpretable and actionable.
...
trainIndex <- createDataPartition(data_updated$Installs_Category, p = 0.8, list = FALSE)
trainData <- data_updated[trainIndex, ]  
testData <- data_updated[-trainIndex, ]
```
...
rf_model <- randomForest(
  Installs_Category ~ Rating + Reviews + Size + Price + content_rating + lastupdate + catART_AND_DESIGN + catGAME + catPRODUCTIVITY + catBUSINESS + catEDUCATION,
  data = trainData,
...
confusionMatrix(rf_predictions, testData$Installs_Category)
```
...
As we can see the boundary is not linear 

### Fit SVM model with gamma = 1 and cost = 1

```{r}
# Fit the SVM model
svmfit <- svm(y_train ~ ., data = as.data.frame(cbind(X_train, y_train)), 
...
#plot(svmfit, data = as.data.frame(cbind(X_train, y_train)))

# Perform PCA on X_train
pca <- prcomp(X_train, scale. = TRUE)
pca_train_data <- as.data.frame(pca$x[, 1:2])  # Use first two principal components
pca_train_data$Target <- y_train

# Fit the SVM model on the PCA-transformed data
svmfit <- svm(Target ~ ., data = pca_train_data, kernel = "radial", gamma = 1, cost = 1)

# Plot the decision boundary
plot(svmfit, pca_train_data)


```


### Tuning to find the best parameter values for C and Gamma
...
cat("Confusion Matrix for Best Model:\n")
print(confusion_matrix)

# Calculate and print training and test error rates
train_error_rate <- mean(predict(svm_model, newdata = train_data) != y_train)
test_error_rate <- mean(predictions != y_test)
cat("Training Error Rate:", train_error_rate, "\n")
cat("Test Error Rate:", test_error_rate, "\n")

# Print mean accuracy for different values of 'C' and 'sigma'
accuracy_by_C_sigma <- svm_model$results[, c("C", "sigma", "Accuracy")]
cat("Accuracy for different C and sigma values:\n")
print(accuracy_by_C_sigma)


```

### Plot MSE , bias and variance for various parameter values

```{r}

# Prepare to collect results
gamma_values <- c(0.5, 1, 2)
cost_values <- c(0.1, 1, 10, 100)
results <- expand.grid(gamma = gamma_values, cost = cost_values)
results$MSE <- NA
results$Bias <- NA
results$Variance <- NA

# Loop over combinations of gamma and cost
for (i in seq_len(nrow(results))) {
  model <- svm(as.numeric(y_train) ~ ., data = as.data.frame(cbind(X_train, y_train)),
               kernel = "radial",
               gamma = results$gamma[i], cost = results$cost[i])
  ...
  # Calculate MSE
  results$MSE[i] <- mean((predictions - as.numeric(y_test))^2)
  
  # Calculate Bias
  bias <- mean(predictions) - mean(as.numeric(y_test))
  results$Bias[i] <- bias
  
  # Calculate Variance
  variance <- var(predictions)
  results$Variance[i] <- variance
}

# Reshape data for plotting
results_long <- tidyr::pivot_longer(results, 
                                     cols = c(MSE, Bias, Variance), 
                                     names_to = "Metric", 
                                     values_to = "Value")

# Plot MSE, Bias, and Variance vs parameters
library(ggplot2)
ggplot(results_long, aes(x = gamma, y = Value, color = factor(cost))) +
  geom_line() + 
  geom_point(size = 3) +
  facet_wrap(~ Metric, scales = "free_y") + 
  labs(title = "MSE, Bias, and Variance vs Gamma for Different Cost Values",
       x = "Gamma", y = "Value", color = "Cost") +
  theme_minimal()
```
