...
Convertion of Price to numerical is required.
...
data_clean1_factor <- data_clean  # Assuming you want to keep the original data intact

data_clean1_factor$Installs <- factor(data_clean$Installs, levels = sorted_values)
...
Review_Category <- cut(data_clean$Reviews, breaks = breaks, right = FALSE, 
...
boxplot( data_clean$Rating~ Review_Category, data = data_clean, 
...
mean_ratings <- tapply(data_clean$Rating, Review_Category, mean, na.rm = TRUE)
...
plot_data <- data.frame(Rating = data_clean$Rating, Review_Category = Review_Category)
...
ggplot(data_clean, aes(x = log(data_clean$Installs), y = Rating)) +
...
```{r}
ggplot(data_clean, aes(x = reorder(Category, log(data_clean$Installs),  FUN = mean), y = log(data_clean$Installs))) +
...
### Last Updated vs Content Rating vs Installs 
```{r}
...
data_numeric_or_factor$Android.Ver <- as.factor(data_numeric_or_factor$Android.Ver)
data_numeric_or_factor$Category <- as.factor(data_numeric_or_factor$Category)
...
# log_Installs = log10(data_final$Installs)
...
## Spearsman correlation for Last Updated vs Installs
```{r}

data_final$days_since_update <- as.numeric(difftime(Sys.Date(), data_final$Last.Updated, units = "days"))
correlation_update_installs <- cor.test(data_final$days_since_update, log10(data_final$Installs), method = "spearman")
...
## Correlation between Last updated VS Installs
```{r}
