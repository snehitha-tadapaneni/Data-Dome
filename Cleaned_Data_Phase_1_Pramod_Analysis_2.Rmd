...
Converting the Last Updated Column values to a uniform format (Year-Month-Date)
...
There are many apps rated for everyone followed by Teen, Mature and Kids
```{r}
...
ggplot(data_clean, aes(x = Content.Rating)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
Frequency of different content ratings assigned to a set of apps, each bar represents the count of apps for a specific content rating category. This Bar plot also shows that there are many apps rated for everyone followed by Teen, Mature and Kids

## Last Updated Analysis

### Create summary of updates by month and year

I wanna create montly summaries to visualize how the updates varies over time. So I created two new columns for month and year by using the mutate() function. I then grouped the data by month
```{r}
...
### Plot of updates over time
This block of code create a line graph shows the increase in the number of updates over time. There is a very low activity from 2010 to 2014, a gradual increase from 2014-2017. And a dramatic increase in app updates around 2018. This plot suggests that the google play store application initially had slow adoption but grew in popularity by time. It also suggests that there were possible platform changes or policy updates in 2018 that triggered massive update activity
```{r}
...
This analysis could reveal interesting patterns, such as:
Whether mature-rated apps get updated more/less frequently than others
Which content rating categories have the most apps
If certain ratings show more consistent update patterns (comparing avg vs median)
```{r}
...
Lets perform chi-square test to analyze if there's a significant relationship between content ratings and update years.

### Creating contingency table of Content Rating vs Update Year
Lets create a contingency table where rows represent different content ratings (e.g., Everyone, Teen, Mature) and columns represent years when apps were last updated.
Each cell shows the count of apps for that rating-year combination
```{r}
...
print(content_update_table)

chi_test <- chisq.test(content_update_table)
print("\nChi-square test results:")
print(chi_test)
```
The P value is very small signifying that there is statistically significant relationship between Content Rating and Last Updated Columns

## Content Rating Basic Analysis by calculating percentages
```{r}
...
## Time-based Analysis
### Content Rating Quarter Analysis
If first created 4 new columns, Year, Month, Quarter of last update and the days passed since the most recent update. Then I plotted a visualization to show the count of each unique value of content rating over four quarters 
```{r}
