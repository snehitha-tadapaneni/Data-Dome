...
```{r}
glm_data <- data_apps
glm_data <- glm_data[, !colnames(glm_data) %in% c("Installs")]
col_to_scale <- c('Rating', 'Reviews', 'Size', 'Price', 'Content.Rating', 'Last.Updated')
glm_data[col_to_scale] <- scale(glm_data[col_to_scale])
```
...
trainIndex <- createDataPartition(glm_data$Installs_Category, p = 0.8, list = FALSE)
trainData <- glm_data[trainIndex, ]
testData <- glm_data[-trainIndex, ]
```

We first start with the basic Logistic model.
```{r}
glm_model <- glm(Installs_Category ~ ., data = trainData, family = binomial)
summary(glm_model)
```

#### Stepwise Selection for GLM
Next, the stepwise selection is proceeded based on the logistic model above. It can be seen that the model below has AIC value of 2258.3, indicating the below model performs better than the above (which has AIC of 2290). 
...
```{r}
glm_probs <- predict(stepwise_model, testData, type = "response")

glm_pred <- ifelse(glm_probs > 0.5, 1, 0)

# Generate confusion matrix
test_cm <- confusionMatrix(factor(glm_pred), factor(testData$Installs_Category))
print(test_cm)
...
# Generate ROC curve
roc_curve <- roc(testData$Installs_Category, glm_probs)
...
```
The AUC for the model is 0.83 which is good. But the performance metrics such as accuracy, precision are anticipated to have an above 80% score.
