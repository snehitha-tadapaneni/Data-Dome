...
roc_curve <- roc(as.numeric(y_test), y_pred_prob[,2])
...
ggplot(importance_df[1:10,], aes(x = reorder(Feature, Importance), y = Importance)) +
  ...
  coord_flip() +
  labs(title = "Top 10 Feature Importance (KNN Model)",
       x = "Feature",
       y = "Importance Score") +
  theme_minimal()
...
pred_df <- data.frame(
  ...
  Predicted_Prob = y_pred_prob[,2]
)

ggplot(pred_df, aes(x = Predicted_Prob, fill = factor(Actual))) +
  geom_density(alpha = 0.5) +
  labs(title = "Distribution of Predicted Probabilities by Actual Class",
       x = "Predicted Probability of High Installs",
       y = "Density",
       fill = "Actual Class") +
  scale_fill_manual(values = c("red", "blue"),
                   labels = c("Low Installs", "High Installs")) +
  theme_minimal()
```

```{r}
# After running the existing model code, add these visualizations:

# 1. Density Plot of Probability Distributions
ggplot(data.frame(
  probability = y_pred_prob[,2],
  actual = y_test
), aes(x = probability, fill = actual)) +
  geom_density(alpha = 0.7) +
  scale_fill_manual(values = c("low" = "#FF9999", "high" = "#66B2FF")) +
...
# 2. Enhanced Confusion Matrix Visualization
...
# 4. Feature Importance Visualization with Categories
importance_df$Category <- factor(
  ifelse(abs(importance_df$Importance) >= quantile(abs(importance_df$Importance), 0.75), "High",
         ifelse(abs(importance_df$Importance) >= quantile(abs(importance_df$Importance), 0.25), "Medium", "Low"))
)

ggplot(head(importance_df, 15), 
       aes(x = reorder(Feature, abs(Importance)), 
           y = abs(Importance),
           fill = Category)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("High" = "#4A90E2", 
                              "Medium" = "#79B6F2", 
                              "Low" = "#A8D1F2")) +
  coord_flip() +
...
radar_data <- data.frame(
  matrix(c(rep(1,5), rep(0,5), values),
         nrow = 3, byrow = TRUE)
)
...
confidence_scores <- apply(y_pred_prob, 1, max)
ggplot(data.frame(confidence = confidence_scores), aes(x = confidence)) +
