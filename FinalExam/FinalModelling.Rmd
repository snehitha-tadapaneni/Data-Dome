...
```
This code begins by selecting relevant features for prediction, including numerical attributes like `Rating` and `Reviews` and categorical features identified by a specific naming pattern. It then splits the dataset into training (70%) and testing (30%) subsets, ensuring the distribution of the target variable remains consistent across both sets. Next, it scales the features using centering and standardization techniques to ensure that numerical variables are normalized with a mean of 0 and a standard deviation of 1, which is critical for models sensitive to feature scales. The scaling parameters are computed on the training data and then consistently applied to both training and testing datasets, preparing the data for accurate and reliable machine learning model training and evaluation.
...
As we can see, the most optimal k is found by using CV and accuracy as metric, highest accuracy of about 78% is achieved. This code trains a K-Nearest Neighbors (KNN) model with cross-validation to determine the optimal value of `k` (number of neighbors). It evaluates different `k` values, ranging from 1 to 20 with a step of 2, using 5-fold cross-validation for each value. The accuracy for each `k` is stored, creating a dataset of `k` values and their corresponding accuracies. Finally, a line plot is generated to visualize the relationship between `k` and cross-validation accuracy, helping to identify the optimal `k` that maximizes accuracy. This visualization aids in selecting the most effective model configuration.
...
```
This code finalizes the KNN model by selecting the optimal number of neighbors (`k`) based on cross-validation. Using this optimized model, predictions are made on the test dataset, both as class labels and probabilities. Performance metrics, including accuracy, precision, recall, F1 score, and AUC, are calculated and summarized. A confusion matrix is visualized as a heatmap, showing the distribution of correct and incorrect predictions, and the ROC curve is plotted to evaluate the model's ability to discriminate between classes, with an AUC score of 0.83 indicating good performance. The top 5 features contributing to the model are Size, Rating, Last updated, Content Rating and Reviews.

Feature importance is then computed using a permutation-based approach, where the impact of shuffling each feature on the model’s AUC is measured. This determines the significance of each feature in contributing to the model’s predictions. The importance scores are visualized in a bar plot, providing insights into the most influential features for the KNN model. This entire process not only evaluates the model but also highlights key features driving its predictions.



### Conclusion
The KNN model shows strong performance, effectively predicting app 
installation categories based on key features like user engagement and 
app characteristics. Features such as reviews, app size, and category 
indicators play a significant role in influencing predictions. The model's 
balanced precision and recall indicate reliable classification, and 
visualizations confirm the model's ability to make confident predictions
with minimal misclassification. Overall, it is a robust solution for 
understanding and categorizing app installation trends.
...
```
So the Random Forest Model performs pretty well and is reliable as the accuracy is 95.17%. 
Also looking at the feature importance plot, we can say that Reviews, Ratings, Size and Last Update 
has the highest contribution in predicting. When it comes to Categories, the top 5 categories are
medical, events, entertainment, photography and gaming.
...
success. Also from the confusion matrix, we can see the true negatives 
are higher compared to false negatives which tells us that the target 
variable Installs category is slightly imbalanced.
